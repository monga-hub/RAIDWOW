...
function profileMilestone(profile){const builds=RAID_PROFILE_ROLES.map(role=>profile?.heroes?.[role]).filter(Boolean),developed=builds.length;if(developed<4)return{rank:'Adunata',current:developed,total:4,label:`${developed}/4 eroi sviluppati`};if(developed<8)return{rank:'Compagnia',current:developed,total:8,label:`${developed}/8 eroi sviluppati`};const levels=builds.map(build=>Math.max(5,Number(build.heroLevel)||5)),minimum=Math.min(...levels),target=Math.max(6,Math.floor(minimum/2)*2+2),current=levels.filter(level=>level>=target).length,rank=minimum>=12?'Leggende delle Paludi':minimum>=10?'Custodi del Tempio':minimum>=8?'Veterani':'Compagnia completa';return{rank,current,total:8,label:`${current}/8 eroi al LV ${target}`}}
...
const RaidProfile={PROFILE_KEY:RAID_PROFILE_KEY,LEGACY_JOURNEY_KEY:RAID_LEGACY_JOURNEY_KEY,LEGACY_CLEARED_KEY:RAID_LEGACY_CLEARED_KEY,ROLES:RAID_PROFILE_ROLES,migrate:migrateRaidProfile,normalize:normalizeRaidProfile,milestone:profileMilestone,load:loadRaidProfile,write:writeRaidProfile,loadJourney:loadProfileJourney,saveJourney:saveProfileJourney,clearJourney:clearProfileJourney,saveHeroes:saveProfileHeroes,recordRun:recordProfileRun,loadCleared:loadProfileCleared,markCleared:markProfileCleared};
