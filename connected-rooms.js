...
function connectedPlacementPanel(g){
  ...
  const doors=tiles.flatMap(tile=>roomTileConnectors(tile).map(side=>`<button class="door-card" data-connect-tile="${tile.id}" data-connect-side="${side.id}" aria-label="Collega ${tile.name||tile.id}, lato ${side.id}"><span class="door-plaque"><strong>${tile.name||tile.id}</strong><small>${tile.id} · Lato ${side.id}</small><span>${iconSummary(side.icons)} · ${roomTileConnectors(tile).length-1} uscite restanti</span></span></button>`)).join('');
  return`<section class="hero connection-placement"><h2>${forced?'Stanza 11 — collega il Trono di Grumara':'Turno Overlord — scegli la porta'}</h2><p>Uscita scelta dagli Eroi: <strong>${exit?.id||'—'}</strong> ${iconSummary(exit?.icons)}</p><div class="connection-tiles">${doors}</div></section>`;
}
